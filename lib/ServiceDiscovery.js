 ...
 *
 */
function ServiceDiscovery (connectionString, basePath, serviceName, options) {	
...
ServiceDiscovery.prototype.registerService = function (address, port, callback) {
  ...
  var id              = uuid();
  var absSvcPath      = [ self.basePath, self.serviceName ].join('/');
  ...
  async.waterfall([
    // Create service path
    function(cb) {
      self.client.mkdirp(absSvcPath, cb);
    },
